...
function DropdownPanel({ subLinks }: { subLinks: SubLink[] }) {
  ...
  return (
    <div className="fixed left-1/2 transition-all duration-500 -translate-x-1/2 mt-8 h-[280px] w-[850px] scale-110 bg-white rounded-2xl shadow-2xl p-6 flex gap-6 z-50">
      {/* Links */}
      
      <div className="flex gap-18 flex-10 justify-center">
        <p className="fixed left-15 mt-6 scale-95 text-slate-500 font-sans">core services</p>
        <div className="justify-center scale-120 flex flex-col gap-1">
          {col1.map((sub) => (
      ...
      {/* Image */}
      <div className="w-[220px] h-[232px] scale-110 rounded-xl overflow-hidden flex-shrink-0 relative">
        <Image
          src={activeImage}
          alt="preview"
          fill
...
>
      ...
      {/* Logo */}
      <div className={`flex text-[26px] z-10 transition-colors duration-500 ${isAtTop ? 'text-white' : 'text-black'}`}>
        <p>Rise at Seve</p>
        <Image
      ...
      {/* Nav links */}
      <ul className="flex items-center z-50 relative">
        {NAV_LINKS.map((link) => (
          <li
            key={link.label}
            className="relative"
          ...
          >
            <a
              href="#"
              className={`font-semibold py-0.5 px-4 text-[15px] rounded-full whitespace-nowrap flex items-center transition-[background-color,color] duration-300 ${
                isAtTop
      ...
      {/* CTA */}
      <button className={`relative font-sans text-[15px] font-semibold px-6 py-3 rounded-[25px] hover:rounded-lg duration-500 transition-all whitespace-nowrap overflow-hidden group ${
